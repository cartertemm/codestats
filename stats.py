...
parser = argparse.ArgumentParser(
	description="Tool to provide detailed summaries of code in a project."
)
...
parser.add_argument(
	...
	action="append",
	default=[],
	help="adds a wildcard exclusion",
)
parser.add_argument(
	"-ef",
	"--exclude-file",
	help="Path to a file containing a list of excludes to be processed seperated by \\n",
)
parser.add_argument(
	"-v",
	"--verbose",
	action="store_true",
	help="give args.verbose/more detailed output",
)
parser.add_argument(
	"-dr",
	"--disable-rundown",
	action="store_true",
	help="excludes an individual rundown of every processed file",
)
...
if args.verbose:
	print(
		"loaded a total of "
		+ str(len(args.excludes))
		+ " exclusion"
		+ ("s" if len(args.excludes) != 1 else "")
	)
...
for i in directory:
	...
	if not lang:
		if args.verbose:
			print(
				"unknown language from file "
				+ i
				+ ". No info will be provided. Continuing"
			)
		continue
	f = open(i, "rb")
	content = f.read()
...
print(
	f"average number of characters per line: {round(line_amounts[1]/line_amounts[0], 2)}"
)
print(f"average number of lines per file: {round(total_lines/len(files), 2)}")
print(
	f"total number of proceeding spaces/indents: {total_indents}, making up a total of {bytes2human(total_indents)}"
)
...
print(
	f"average file size: {bytes2human(total_size/len(files))}, {round(total_size/len(files), 2)}b"
)
print(
	f"average file size without indents: {bytes2human(without_indents/len(files))}, {round(without_indents/len(files), 2)}b"
)
...
if not args.disable_rundown:
	...
	for i in files:
		print(
			f"{i}, {files[i][0].name}, {files[i][1]} lines, {bytes2human(files[i][2])}, {files[i][2]}b"
		)
